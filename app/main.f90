...
!> Command-line driver for dimer projection methods
program main
   ...
   use frag, only : get_jab, jab_input
   use frag_version, only : get_frag_version
   implicit none
   character(len=*), parameter :: prog_name = "frag"

   type, extends(jab_input) :: driver_config
   ...
   call get_jab(config%jab_input, mol, error)
   if (allocated(error)) then
      write(error_unit, '(a)') error%message
      error stop
   end if
...
subroutine version(unit)
   ...
   call get_frag_version(string=version_string)
   write(unit, '(a, *(1x, a))') &
